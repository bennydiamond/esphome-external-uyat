...
struct DpDimmer
{
   ...
   DpDimmer(BrightnessChangedCallback callback, MatchingDatapoint dimmer_dp, const uint32_t min_value, const uint32_t max_value, std::optional<MatchingDatapoint> min_value_dp = std::nullopt):
   callback_(callback),
   ...
   {
      ESP_LOGCONFIG(DpDimmer::TAG, "   Dimmer is %s", dp_dimmer_.get_config().to_string().c_str());
      ESP_LOGCONFIG(DpDimmer::TAG, "   Min value: %u", min_value_);
      ESP_LOGCONFIG(DpDimmer::TAG, "   Max value: %u", max_value_);
      if (dp_min_value_.has_value())
      {
         ESP_LOGCONFIG(TAG, "   Has min_value_datapoint: %s", dp_min_value_->get_config().to_string().c_str());
      }
   }

   void set_value(const float value_percent)
   {
      ...
      ESP_LOGD(DpDimmer::TAG, "Setting dimmer, brightness %.3f, brightness_int %d", value_percent, brightness_int);
...
private:
   ...
   {
      ESP_LOGV(DpDimmer::TAG, "MCU reported dimmer %s is: %.4f", get_object_id().c_str(), value);
